...
前后端分离输出要求（严格）：
1. 必须使用 HTML <table> 格式输出，包含三列：功能点 | 前端用例 | 后端用例。
2. 功能点按业务功能粒度划分（如”登录功能”、”订单管理”），不要按单个用例拆行。
3. 同一功能点的前后端用例必须在同一 <tr> 中。
4. 单元格内保持嵌套列表格式（加粗测试点 + 编号步骤 + 预期结果）。
5. 前端列仅包含：页面展示、交互流程、UI层输入校验、按钮状态、前端路由、前端错误提示、可用性与兼容性。
6. 后端列仅包含：接口契约、鉴权、业务规则、幂等、事务一致性、并发、数据落库、异常码与日志。
7. 禁止在前端列出现”查库/事务/落库”等后端实现步骤；禁止在后端列出现”点击页面/按钮/弹窗”等前端操作步骤。
8. 若某功能点仅涉及单端，另一列填”无”。

输出格式示例：
<table>
<tr><th>功能点</th><th>前端用例</th><th>后端用例</th></tr>
<tr>
<td>登录功能</td>
<td>
...
- **登录接口验证**
  1. 发送POST /api/login，传入有效凭证
  2. 检查返回状态码和token
    - 返回200，body含有效JWT token
...
前后端分离评审要求（严格）：
1. 检查输出是否为 HTML <table> 格式，包含 <table>、</table> 标签。
2. 检查表头是否包含三列：功能点、前端用例、后端用例。
3. 检查每行是否按功能点组织，同一功能点的前后端用例在同一 <tr> 中。
4. 检查前端列是否混入后端实现步骤（如查库、事务、落库）。
5. 检查后端列是否混入前端交互步骤（如点击页面、按钮、弹窗）。
6. 检查是否存在跨端遗漏（某功能点只有前端用例却缺少对应后端用例，反之亦然）。
7. 对格式或混写错误给出可执行修正建议。
"""

SPLIT_FE_BE_OPTIMIZER_INSTRUCTION = """
前后端分离优化要求（严格）：
注意：忽略上方嵌套列表示例格式，按以下 HTML 表格格式输出最终结果。

1. 最终结果必须使用 HTML <table> 格式，三列：功能点 | 前端用例 | 后端用例。
2. 功能点按业务功能粒度划分，不要按单个用例拆行。
3. 必须修复所有跨列混写问题，确保前后端步骤彻底分离。
4. 若某条用例同时涉及前后端，拆分为两条分别放入对应列。
5. 若某功能点仅涉及单端，另一列填”无”。
6. 若评审反馈与分离规则冲突，以分离规则为最高优先级。
